...
import { getSocialAuthUrl } from '../../store/slices/authSlice';

interface SocialLoginProps {
  showTitle?: boolean;
}

const SocialLogin: React.FC<SocialLoginProps> = ({ showTitle = true }) => {
  ...
  const handleSocialLogin = async (provider: string) => {
    try {
      ...
      const result = await dispatch(getSocialAuthUrl(provider)).unwrap();
      console.log('Social auth URL received:', result);
      ...
      if (error.message?.includes('OAuth configuration missing')) {
        alert(`Please configure ${provider} OAuth credentials in the server. Contact administrator.`);
      } else {
